...
(function (window, document) {
  ...
  Croppic.prototype = {
    ...
    createImgUploadControls: function () {
      ...
      if (!$.isEmptyObject(that.croppedImg)) {
        that.cropControlRemoveCroppedImage = that.outputDiv.find(
          '.cropControlRemoveCroppedImage'
        );
      }
    },
    bindImgUploadControl: function () {
      ...
      if (!$.isEmptyObject(that.croppedImg)) {
        that.cropControlRemoveCroppedImage.on('click', function () {
          if (
            typeof that.options.onBeforeRemoveCroppedImg === typeof Function
          ) {
        ...
        if (that.options.processInline) {
          ...
          if (typeof FileReader == 'undefined') {
            ...
            reader.readAsDataURL(
              that.form.find('input[type="file"]')[0].files[0]
            );
          }
        } else {
          try {
    ...
    createEyecandy: function () {
      ...
      that.imgEyecandy
        .css({ 'z-index': '0', opacity: that.options.imgEyecandyOpacity })
        .appendTo(that.obj);
    },
    ...
    createCropControls: function () {
      ...
      if (that.options.doubleZoomControls) {
        that.cropControlZoomMuchIn = that.cropControlsCrop.find(
          '.cropControlZoomMuchIn'
        );
        ...
        that.cropControlZoomMuchOut = that.cropControlsCrop.find(
          '.cropControlZoomMuchOut'
        );
      ...
      that.cropControlZoomOut = that.cropControlsCrop.find(
        '.cropControlZoomOut'
      );
      ...
      that.cropControlZoomIn = that.cropControlsCrop.find(
        '.cropControlRotateLeft'
      );
      that.cropControlZoomIn.on('click', function () {
      ...
      if (typeof FormData == 'undefined') {
        ...
        for (var key in cropData) {
          urlEncodedDataPairs.push(
            encodeURIComponent(key) + '=' + encodeURIComponent(cropData[key])
          );
        }
    ...
    CreateFallbackIframe: function () {
      ...
      if (!that.isAjaxUploadSupported()) {
        ...
        that.iframeform = that.iframeobj
          .contents()
          .find('html')
          .find('.' + that.id + '_upload_iframe_form');
        ...
        var eventHandlermyFile = function () {
          ...
          else iframe.removeEventListener('load', eventHandlermyFile, false);
    ...
    getIframeContentJSON: function (iframe) {
      try {
        ...
        if (
          innerHTML.slice(0, 5).toLowerCase() == '<pre>' &&
